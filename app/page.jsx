...
function Grid({ packs }) {
  if (!packs?.length) return <div>No packs loaded.</div>;
  return (
    <div className="cards">
...
function PackCard({ pack }) {
  const img = pack?.metadata?.image || pack?.image || "";
  const title = pack?.collectionName || "Pack";
  const price = pack?.priceUsd ?? pack?.priceUSD ?? pack?.price_usd;
  const perPack = price ? fmt.usd(price) : (pack?.priceEth ? fmt.eth(pack.priceEth) : "-");
    ...
    <div className="card">
      ...
      <div className="meta">
        ...
        <div className="row-between">
          <div className="sub">{pack?.creator?.handle ? `@${pack.creator.handle}` : "—"}</div>
          <div className="sub">{perPack} / pack</div>
        </div>
        <div className="row">
          <a className="btn" href={`https://vibechain.com/market?tab=recent`} target="_blank" rel="noreferrer">Open</a>
          <BuyTokenButton pack={pack} />
        </div>
      </div>
    </div>
  );
...
function PackSmall({ pack }) {
  ...
  return (
    <div className="card">
      <div className="thumb" style={{height:72, backgroundImage:`url(${img})`}} />
      <div className="meta">
        <div className="title">{title}</div>
        <div className="row-between">
          <div className="sub">{perPack}</div>
          <a className="btn ghost" href={`https://vibechain.com/market`} target="_blank" rel="noreferrer">View</a>
        </div>
      </div>
    </div>
  );
...
function BuyTokenButton({ pack }) {
  // Om vi får tokenAddress i pack.metadata token, använd den.
  const token = pack?.tokenAddress || pack?.metadata?.tokenAddress || "";
  if (!token) return <button className="btn ghost" disabled>Buy token</button>;

  const url = `https://app.uniswap.org/swap?inputCurrency=ETH&outputCurrency=${token}&chain=base`;
  return (
    <a className="btn" href={url} target="_blank" rel="noreferrer">Buy token</a>
  );
}

function QuickBuy({ token }) {
  if (!token) return <div className="sub">Set NEXT_PUBLIC_TNYL_TOKEN to enable quick buy.</div>;
  const url = `https://app.uniswap.org/swap?inputCurrency=ETH&outputCurrency=${token}&chain=base`;
  return (
    <a className="btn" href={url} target="_blank" rel="noreferrer">Buy $TNYL</a>
  );
}
  ...
  return (
    <div className="card">
      <div className="meta">
        ...
        <div className="sub">Hook in endpoint(s) as they open up.</div>
      </div>
    </div>
  );
}
