...
export default function Page() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // PACKS (marketplace)
        const packsRes = await wieldFetch(
          `vibe/boosterbox/recent?limit=120&includeMetadata=true&chainId=${CHAIN_ID}`
        );
        ...
        let actList = [];
        try {
          const openings = await wieldFetch(
            `vibe/openings/recent?limit=60&includeMetadata=true&chainId=${CHAIN_ID}`
          );
          actList = openings?.data || openings || [];
        } catch {
          const fb = await wieldFetch(
            `vibe/boosterbox/recent?limit=120&includeMetadata=true&status=opened&chainId=${CHAIN_ID}`
          );
          actList = fb?.data || fb || [];
        }
  ...
  // Verified creators – sorterade efter högsta USD-värde på något av deras packs
    ...
    <>
      ...
      <header className="header">
        <div className="row">
          <div className="logo">
            ...
            <div>
              VibeMarket <span style={{ opacity: 0.7 }}>Tracker</span>
            </div>
            <div className="pill">VibePoints</div>
          </div>
          <div className="row">
            <div className="wallet-chip">{wallet ? short(wallet) : "Not Connected"}</div>
            <button className="btn" onClick={connectWallet(setWallet)}>
              Connect
            </button>
      ...
      <div className="wrapper">
        <div className="grid">
          <div>
              ...
              <section className="panel">
                <div className="panel-head">
                  ...
                  <div className="row">
                    ...
                    <a className="btn ghost" href="https://vibechain.com/market" target="_blank" rel="noreferrer">
                      Open VibeMarket
                    </a>
                  </div>
                </div>
            ...
            {active === "Activity" && (
              <section className="panel">
                <div className="panel-head">
                  ...
                  <button className="btn" onClick={refreshActivity(setTicker)}>
                    Refresh
                  </button>
                </div>
              ...
              <section className="panel">
                <div className="panel-head">
                  ...
                  <div className="row">
                    ...
                    />
                    <button className="btn" onClick={() => loadProfile(wallet, setBoughtItems)}>
                      Load
                    </button>
                  </div>
                </div>
                <div className="cards">
                  <div className="card">
                    PnL &amp; holdings kopplas till <code>/vibe/owner/:address</code> när endpointen är öppen.
                  </div>
                </div>
              </section>
            )}
            ...
            {active === "Bubble" && (
              <section className="panel">
                <div className="panel-head">
                  <div className="panel-title">Bubble Map</div>
                </div>
                <div className="cards">
        ...
        <div className="footer">
          Built for the Vibe community • Prototype •{" "}
          <a href="https://vibechain.com/market" target="_blank" rel="noreferrer">
            VibeMarket
          </a>{" "}
          • Created by{" "}
...
function PackCard({ pack }) {
  ...
  return (
    <a className="card pack" href={link} target="_blank" rel="noreferrer">
      {img ? (
        <img className="thumb lg" alt="" src={img} />
      ) : (
        <div className="thumb lg" style={{ background: "var(--bg-1)" }} />
      )}
      <div className="meta">
        <div className="row-between">
          <div className="title">
            {name} {verified && <span className="badge">VERIFIED</span>}
          </div>
    ...
    <section className="panel">
      ...
      <div className="row" style={{ marginBottom: 10 }}>
        ...
        <input
          ...
          value={form.contract}
          onChange={(e) => setForm((s) => ({ ...s, contract: e.target.value }))}
        />
        <input
          className="input"
          placeholder="Token ID"
          value={form.tokenId}
          onChange={(e) => setForm((s) => ({ ...s, tokenId: e.target.value }))}
        />
      ...
      <div className="cards" style={{ gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))" }}>
        {!items.length ? (
          <div className="card">Inga items i din trade-lista ännu.</div>
        ) : (
          items.map((i) => (
...
function loadTradeList() {
  try {
    return JSON.parse(localStorage.getItem("forTrade") || "[]");
  } catch {
    return [];
  }
}
function saveTradeList(list) {
  try {
    localStorage.setItem("forTrade", JSON.stringify(list || []));
  } catch {}
}
